...
func (u *RestController) RestMake(c *gin.Context){
	...
	if input["lat"] == "" {panic("no lat sent")}
	if input["long"] == "" {panic("no long sent")}
	if input["owner"] == "" {panic("no owner sent")}
...
func (u *RestController) Restget(c *gin.Context){
	...
	if input["rid"] == "" {panic("no rid sent")}
...
func (u *RestController) RestUpdate(c *gin.Context){
	...
	if input["update1"] == "" {panic("no update sent")}
